...
module.exports = function(options) {
	...
	obj.post = function(partialUrl, params, next) {
		...
		if (params && !params.developerApplication) params.developerApplication = config.developerApplication;
	...
	//Tokenization of a card is browser only feature.
	//This package supports tokenization here to enable
	//full workflow testing. Use the vault methods on the server.
	obj.browserTokenizationForUnitTestingOnly = function(partialUrl, params, next){

		var url = obj.baseUrl() + partialUrl;
		var options = {
			method: 'POST',
			json: params,
			jar: true,
			headers: {'Origin': 'https://localhost:4343'},
		};
		if (params && !params.developerApplication) params.developerApplication = config.developerApplication;
		return request(url, options, wrapCallback(next));
	};
